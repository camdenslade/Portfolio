...
const KIMBU_BASE_URL = (0, params_1.defineString)('KIMBU_BASE_URL', { default: 'https://api.kimbu.cslade.space' });
const KIMBU_APP_ID = (0, params_1.defineString)('KIMBU_APP_ID');
async function introspect(token) {
    const res = await fetch(`${KIMBU_BASE_URL.value()}/v1/auth/introspect`, {
        method: 'POST',
...
exports.setBadgeOverrides = (0, https_1.onRequest)({ cors: ['https://cslade.space', 'https://portfolio-dae1f.web.app', 'http://localhost:3000'] }, async (req, res) => {
    ...
    await db.collection('portfolio').doc('badge-overrides').set({ overrides, updatedAt: admin.firestore.FieldValue.serverTimestamp(), appId: KIMBU_APP_ID.value() }, { merge: false });
